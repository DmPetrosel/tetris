    ...
    for (int x = 0; x < COLS_FIELD; x++) {
      if (y - rows_quantity > 0)
        state->current_field.field[y][x] =
            state->current_field.field[y - rows_quantity][x];
      else
...
void check_strike() {
  ...
  if (end) shift_field_down(end, count);
  ...
  if (state->game_info.score / 600 > state->game_info.level) {
    ...
    if (state->game_info.speed - SPEED_STEP >= SPEED_MIN) {
...
int can_be_moved(int delta_row, int delta_col) {
  ...
  for (int i = 0; i < BRICK_N && res; i++) {
    for (int j = 0; j < BRICK_N && res; j++) {
      if ((x + j < 0 || x + j >= COLS_FIELD || y + i >= ROWS_FIELD) &&
          state->current_brick.matrix[i][j]) {
        res = 0;
      } else if ((state->current_field.field[y + i][x + j] &&
                  state->current_brick.matrix[i][j] &&
                  (!check_brick(delta_row, delta_col, i, j)))) {
        res = 0;
      }
    }
  }
...
void varnish() {
  Game_t *state = game_init();
  for (int y = CUR_BRICK_Y, i = 0; y < BRICK_N + CUR_BRICK_Y && ALLOWED;
       y++, i++) {
    for (int x = CUR_BRICK_X, j = 0; x < BRICK_N + CUR_BRICK_X && ALLOWED;
         x++, j++) {
...
void generate_figure(Brick_t *brick) {
  ...
  // }
  for (int i = 0; i < BRICK_N; i++)
    for (int j = 0; j < BRICK_N; j++) brick->matrix[i][j] = 0;
  srand(time(NULL));
